...
<Cards>
  <Card
    title="Python SDK"
    description="Full-featured SDK for Python 3.9+ with async support, decorators, and LLM integrations"
    href="/docs/sdk/python"
  />
  <Card
    title="JavaScript SDK"
    description="TypeScript-first SDK for Node.js 20+ with ESM and CommonJS support"
...
const client = new Brokle({
  ...
  flushInterval: 10,           // Optional: Flush interval (seconds)
  debug: false,                // Optional: Enable debug logging
});
```
...
|---------|--------|------------|
| Callback / Context Manager | `with client.start_as_current_span()` | `client.startActiveSpan(name, fn)` |
| Decorator | `@observe(name="...")` | `observe({ name: "..." }, fn)` |
| Manual | `span.update()`, `span.end()` | `client.updateCurrentSpan()`, `span.end()` |
...
|------|-------------|--------|------------|
| `span` | General operation | `start_as_current_span()` | `startActiveSpan()` |
| `generation` | LLM calls | `start_as_current_generation()` | `startActiveGeneration()` |
...
| `flush_interval` / `flushInterval` | float/number | `5.0` / `10` | Auto-flush interval (seconds) |
