...
- A Brokle account ([sign up for free](https://app.brokle.com))
...
1. Log in to [app.brokle.com](https://app.brokle.com)
...
1. Open [app.brokle.com](https://app.brokle.com)
...
<Tabs>
  ...
  <Tab value="javascript" label="JavaScript">
    ...
    // Create a trace with context
    await brokle.startActiveSpan('customer_support_chat', async (span) => {
      span.setAttribute('userId', 'user_123');
      span.setAttribute('feature', 'support_bot');
...
<Tabs>
  ...
  <Tab value="javascript" label="JavaScript">
    ...
    await brokle.startActiveGeneration('chat_completion', 'gpt-3.5-turbo', 'openai', async (span) => {
      const response = await client.chat.completions.create({
        model: 'gpt-3.5-turbo',
        messages: [{ role: 'user', content: 'Hello!' }]
      });
