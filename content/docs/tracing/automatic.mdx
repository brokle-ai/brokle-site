...
<Tabs>
  ...
  <Tab value="javascript" label="JavaScript">
    ```javascript
    await brokle.startActiveSpan('customer_inquiry', async (span) => {
      span.setAttribute('customerTier', 'enterprise');
      span.setAttribute('inquiryType', 'billing');
      ...
      brokle.updateCurrentSpan({ output: response.choices[0].message.content });
    });
...
Wrapper functions like `wrap_openai()` accept only the client — all configuration is set globally via the `Brokle` client:
...
<Tabs>
  <Tab value="python" label="Python">
    ...
    # Configure globally — the wrapper uses this automatically
    brokle = Brokle(
        api_key="bk_...",
        mask=my_mask_fn,       # Privacy: mask sensitive data
        sample_rate=0.5,       # Sample 50% of traces
        tracing_enabled=True,  # Master tracing switch
    )

    # Wrapper only takes the client, no config options
    openai_client = wrap_openai(openai.OpenAI())
    ```
  </Tab>
  <Tab value="javascript" label="JavaScript">
...
| Option | Where to Set | Description |
|--------|-------------|-------------|
| `mask` | `Brokle(mask=fn)` | Function to mask sensitive data (PII, credentials) before transmission |
| `enabled` | `Brokle(enabled=False)` | Disable SDK completely (no data sent) |
| `tracing_enabled` | `Brokle(tracing_enabled=False)` | Disable tracing while keeping other features |
| `sample_rate` | `Brokle(sample_rate=0.1)` | Sample only a fraction of traces |
...
wrapped_client = wrap_openai(azure_client)
```
