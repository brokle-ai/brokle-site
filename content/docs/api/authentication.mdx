...
```bash
curl -X POST https://api.brokle.ai/v1/auth/validate-key \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST https://api.brokle.ai/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST https://api.brokle.ai/api/v1/auth/register \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
...
```bash
curl -X GET https://api.brokle.ai/api/v1/projects \
...
```bash
curl -X POST https://api.brokle.ai/api/v1/auth/refresh \
  -H "Content-Type: application/json" \
  -d '{
    "refreshToken": "rt_abc123..."
  }'
...
```bash
curl -X POST https://api.brokle.ai/api/v1/auth/logout \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
```
