...
<Tabs>
  <Tab value="curl" label="cURL">
    ```bash
    curl -X GET "https://api.brokle.ai/api/v1/spans?projectId=proj_123&type=generation&model=gpt-4" \
  ...
  <Tab value="python" label="Python">
    ...
    response = requests.get(
        "https://api.brokle.ai/api/v1/spans",
        headers={
  ...
  <Tab value="javascript" label="JavaScript">
    ...
    const response = await fetch(
      'https://api.brokle.ai/api/v1/spans?' + new URLSearchParams({
        projectId: 'proj_123',
        type: 'generation',
        model: 'gpt-4',
        minDuration: '1000'
...
```
GET /api/v1/spans/aggregate
```

Aggregate span data for analytics.

```bash
curl -X GET "https://api.brokle.ai/api/v1/spans/aggregate?projectId=proj_123&groupBy=model&metric=count" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIs..."
```
...
```
GET /api/v1/spans/search
```

Search across span inputs and outputs.

```bash
curl -X GET "https://api.brokle.ai/api/v1/spans/search?projectId=proj_123&q=error+handling" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIs..."
```
