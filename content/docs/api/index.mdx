...
<Tabs>
  ...
  <Tab value="javascript" label="JavaScript">
    ```javascript
    const response = await fetch('https://api.brokle.ai/v1/traces', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-API-Key': 'bk_your_api_key'
...
```
GET /api/v1/traces?page=2&pageSize=50&sortBy=startTime&sortOrder=desc
```
...
exporter = OTLPSpanExporter(
    endpoint="https://api.brokle.ai/v1/traces",
