...
<Tabs>
  ...
  <Tab value="javascript" label="JavaScript">
    ...
    await client.startActiveSpan('generate_response', async (span) => {
      const response = await generate(prompt);

      span.setAttribute('evalRelevance', 0.95);
      span.setAttribute('evalHelpfulness', 0.88);
      span.setAttribute('evalAccuracy', 0.92);
