...
export async function sendMessage(message: string) {
  ...
  return brokle.startActiveSpan('chat_action', async (span) => {
      ...
      // Set user context
      if (userId) {
        brokle.updateCurrentSpan({ metadata: { userId, sessionId } });
      }
...
export async function POST(request: NextRequest) {
  ...
  try {
    // wrapOpenAI automatically creates a generation span that
    // tracks the full streaming lifecycle, including token usage
