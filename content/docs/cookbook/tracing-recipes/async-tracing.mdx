...
<Tabs>
  ...
  <Tab value="javascript" label="JavaScript">
    ...
    async function processSingleItem(item) {
      return brokle.startActiveGeneration('process_item', 'gpt-4o-mini', 'openai', async (span) => {
        brokle.updateCurrentSpan({ metadata: { item } });
    ...
    async function processBatch(items) {
      return brokle.startActiveSpan('batch_process', async (span) => {
